// Deploy the PlatformPaymaster implementation contract (run once).
// All future platform paymasters are cheap clones of this implementation.
//
// Run: npx hardhat run scripts/deployImplementation.ts --network sepolia
//
// Required .env:
...
async function main() {
  ...
  console.log("─────────────────────────────────────────────");
  console.log("\nNext step:");
  console.log("  Add to .env:  PAYMASTER_IMPLEMENTATION=" + implAddress);
  console.log("  Then run:     npx hardhat run scripts/deployFactory.ts --network sepolia");
}
