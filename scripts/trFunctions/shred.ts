// shred(bytes calldata _remark)
// Sends a UserOp through Pimlico to call shred() on the TR contract.
//
// Run: npx hardhat run scripts/trFunctions/shred.ts --network sepolia
//
// Required .env:
...
async function main() {
  ...
  const txHash = await smartAccountClient.sendTransaction({
    to: contract,
    value: 0n,
    data: encodeFunctionData({ abi, functionName: "shred", args: [remark] }),
  });

  console.log("txHash:", txHash);
  console.log(`\nshred() complete — owner: ${ownerAddress} ✓`);
}
